<script lang="ts">
    ...
    let headlines = StartedGame.headlines;
    const player = StartedGame.player;
    ...
    onMount(() => {
        ...
        (function updateGame(time = 0) {
            ...
            StartedGame.update(Math.floor(time * 1000));
            headlines = StartedGame.headlines;
        })();
...
<div id="app">
    <div class="headlinr">
        <div class="pause-play">
            <h2
                v-if="pause"
                class="btn backLightPurple"
                v-on:click="pause=false">
        ...
        <div class="game">
            <div class="column left">
                <div class="panel raised">
                    ...
                    <h1 class="mediumPurple">
                        {player.commentsTotal}
                        <p>Comments</p>
                    </h1>
                    <h1>
                        {StartedGame.totalConnections}
                        <p>Connections</p>
                    </h1>
                    <hr />
                    <h2>Fame Points:</h2>
                    <h1>{player.points}</h1>
                    <h5
                        class="btn backYellow fullWidth"
                        v-on:click="upgrades = true">
                        Spend Points!
                    </h5>
                </div>
            </div>
                ...
                <div class="userInput">
                    <div class="headline-submission">
                        <div class="thumbnail large">
                            <svg
                                class="portrait"
                                data-jdenticon-value={player.firstName} />
                        </div>
                        <input
                            type="text"
                    ...
                    <h5
                        class="btn backPurple fullWidth"
                        v-on:click="submitHeadline">
                        Add Headline
                    </h5>
                </div>
                        ...
                        <div class="commentSection">
                            {#each item.comments as comment}
                                <div
                                    ...
                                    <div class="content">
                                        <h4>
                                            {comment.commentName.first}
                                            {comment.commentName.last}
                                        </h4>
            ...
            <div class="column right">
                ...
                <div v-if="noTrend" class="error-popup yellow raised">
                    <h2>
                        There is no trend in your headline! (Pick from the
                        right)
                    </h2>
                ...
                <div class="trends-panel">
                    <h2>Trending Now:</h2>
                    {#each StartedGame.trends as item}
                        <h6>{item}</h6>
                    {/each}
                    <h3 class="mgn-top-10 mgn-btm-10">Change the trends:</h3>
                    <h2
                        class="btn backGreen fullWidth"
                        v-on:click="changeTrends">
                        {StartedGame.trendsCost} Points
                    </h2>
                </div>
            </div>
        </div>
