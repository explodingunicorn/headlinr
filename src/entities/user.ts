...
export function User(game, group) {
    //Age, sex, name
    ...
    if (sex > 4) {
        this.pic.link =
            "/boys/boy (" + Math.floor(Math.random() * 60 + 1) + ")";
        console.log("boy");
    } else {
        this.pic.link =
            "/girls/girl (" + Math.floor(Math.random() * 60 + 1) + ")";
        console.log("girl");
    }
    var self = this;
    ...
    function checkHeadlines(headlines, user, game, name) {
        ...
        for (var i = 0; i < postsToCheck; i++) {
            ...
            if (sentiment > 0 && userFeeling > 5) {
                ...
                var total =
                    userFeeling + sentiment + playerFactor - repeatFactor;
            ...
            //If the user feels negatively towards the trend, there's a chance to dislike
                ...
                var total =
                    userFeeling + 5 + sentiment - playerFactor - repeatFactor;
    ...
    function createHeadline(game, user, name) {
        if (aggression >= headlineChance()) {
            var trend =
                game.trends[Math.floor(Math.random() * game.trends.length)];
